...
class TSPDataset(Dataset):
    ...
    def __init__(self, size=50, num_samples=1e6, seed=None, rps = None):
        ...
        self.dataset = torch.rand((num_samples, 5, size))
        self.weight = torch.rand((num_samples, 3, 1))
        self.weightsum = self.weight.sum(1).unsqueeze(2)
        self.weightsum = self.weightsum.expand(num_samples, 3, 1)
        self.weight = self.weight / self.weightsum

        self.weight = self.weight.expand(num_samples, 3, size)
        self.dataset = torch.cat((self.dataset, self.weight), dim=1)
...
def reward(static, tour_indices, w1=1, w2=0):
    ...
    obj3 = tour_altitude.sum(1).detach()
    
    obj = y_weight1[:, 0, 0]*obj1 + y_weight2[:, 0, 0]*obj2 + y_weight3[:, 0, 0]*obj3
    cosv = ((1-y_weight1[:, 0, 0])*obj1 + (1-y_weight2[:, 0, 0])*obj2 + (1-y_weight3[:, 0, 0])*obj3) / torch.sqrt((torch.pow((1-y_weight1[:, 0, 0]), 2) + torch.pow((1-y_weight2[:, 0, 0]), 2) + torch.pow((1-y_weight3[:, 0, 0]), 2)) * (torch.pow(obj1, 2) + torch.pow(obj2, 2) + torch.pow(obj3, 2)))

    return obj, obj1, obj2, obj3, cosv, y_weight3[:, 0, 0]
