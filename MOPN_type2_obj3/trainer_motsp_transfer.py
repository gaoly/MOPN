...
def validate(data_loader, actor, reward_fn, w1, w2, render_fn=None, save_dir='.',
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        reward, obj1, obj2, obj3, cosv, weight3 = reward_fn(static, tour_indices, w1, w2)

        rewards.append(torch.mean(reward.detach()).item())
        obj1s.append(torch.mean(obj1.detach()).item())
        obj2s.append(torch.mean(obj2.detach()).item())
        obj3s.append(torch.mean(obj3.detach()).item())
    ...
    actor.train()
    return np.mean(rewards), np.mean(obj1s), np.mean(obj2s), np.mean(obj3s)
...
def train(actor, critic, w1, w2, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            reward, obj1, obj2, obj3, cosv, reward0 = reward_fn(static, tour_indices, w1, w2)
            ...
            if (batch_idx + 1) % 200 == 0:
                ...
                mean_obj3 = np.mean(obj3s[-100:])
                print('  Batch %d/%d, reward: %2.3f, cosv: %2.3f, obj1: %2.3f, obj2: %2.3f, obj3: %2.3f, loss: %2.4f, took: %2.4fs' %
                      (batch_idx, len(train_loader), mean_reward, mean_cosv, mean_obj1, mean_obj2, mean_obj3, mean_loss,
                       times[-1]))
        ...
        mean_valid, mean_obj1_valid, mean_obj2_valid, mean_obj3_valid = validate(valid_loader, actor, reward_fn, w1, w2, render_fn,'.', num_plot=5)
        ...
        # Update previous obj
        print('Mean epoch loss/reward: %2.4f, %2.4f, %2.4f, obj1_valid: %2.3f, obj2_valid: %2.3f, obj3_valid: %2.3f. took: %2.4fs '\
              '(%2.4fs / 100 batches)\n' % \
              (mean_loss, mean_reward, mean_valid, mean_obj1_valid, mean_obj2_valid, mean_obj3_valid, time.time() - epoch_start,
              np.mean(times)))
    print("Total run time of epoches: %2.4f" % (time.time() - start_total))



def train_tsp(args, rps, w1=1, w2=0, checkpoint = None):
    ...
    train_data = TSPDataset(args.num_nodes, args.train_size, args.seed, rps)
    valid_data = TSPDataset(args.num_nodes, args.valid_size, args.seed + 1, rps)
...
def generate_recursive_3D(rps, pt, num_objs, left, total, element):
    for i in range(left):
        pt.append(-1)
    if element == num_objs-1:
        ...
        for i in range(left+1):
            pt[element] = i/total
            generate_recursive_3D(rps, pt, num_objs, left-i, total, element+1)
            
def get_RPset(rps, objnum):
    RPlist = np.zeros((len(rps), objnum))
    for i in range(len(rps)):
        for j in range(objnum):
            RPlist[i][j] = rps[i][j]

    weightsum = RPlist.sum(1)
    for i in range(len(rps)):
        for j in range(objnum):
            RPlist[i][j] = RPlist[i][j]/weightsum[i]
    return RPlist
...
if __name__ == '__main__':
    ...
    objnum = 3
    divnum = 13
    generate_recursive_3D(rps, pt, objnum, divnum, divnum, 0)
    rps = get_RPset(rps, objnum)
    ...
    for j in range(5):
        ...
        if args.task == 'tsp':
            ...
            checkpoint = ''
            train_tsp(args, rps, 1, 0, checkpoint)
